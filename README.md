...
| setscore                            | Set the current team scores and synchronize the number of rounds played.<br/>Args: `<ct_score> <t_score>` (values from `0` to `100`).<br/>Usage: `setscore 5 7` |
...
| mp_combat_report                   | 0       | 0   | 1            | Show each human player a per-opponent combat report in chat when the next round begins.<br/>`0` disabled<br/>`1` enabled |
...
| mp_buy_allow_shield                | 1       | 0   | 1            | Allow players to buy the tactical shield.<br/>`0` disabled<br/>`1` enabled<br/>`NOTE`: Existing shields, map pickups and items granted by plugins are unaffected. |
...
| mp_pistol_air_accuracy             | 0       | 0   | 1            | Controls pistol accuracy while airborne.<br/>`0` pistols receive the default airborne inaccuracy<br/>`1` pistols use normal movement and stance accuracy while airborne |
...
| mp_round_kill_sounds               | 1       | 0   | 1            | Plays a private sound to the killer based on their number of enemy kills during the current round. The fifth sound is reused for subsequent kills. |
